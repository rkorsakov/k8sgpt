...
var languagePrompts = map[string]LanguagePrompts{
	...
	"ru": {
		"default_prompt": Prompt{Template: `Упрости следующее сообщение об ошибке Kubernetes.
				...
				Выведи результат в следующем формате, проверяй чтобы нигде не терялись пробелы, текст был на русском и названия были правильными(если нужно, можно оставить их на английском):
		...
		"prom_conf_prompt": Prompt{Template: `Упрости следующее сообщение об ошибке Prometheus.
				...
				Выведи результат в следующем формате, не более 300 символов, проверяй чтобы нигде не терялись пробелы, текст был на русском и названия были правильными(если нужно, можно оставить их на английском):
				Ошибка: {Подробное объяснение ошибки}
				Решение: {Подробное решение по пунктам}`},
		...
		"prom_relabel_prompt": Prompt{Template: `Верни ответ, начиная с:
				...
				Проверяй чтобы нигде не терялись пробелы, текст был на русском и названия были правильными(если нужно, можно оставить их на английском)`},
