...
var languagePrompts = map[string]LanguagePrompts{
	...
	"ru": {
		...
		"kyverno_prompt": Prompt{Template: `Упрости следующее предупреждение Kyverno.
				...
				Выведи результат в следующем формате, для решения покажи только команду kubectl, проверяй чтобы нигде не терялись пробелы, текст был на русском и названия были правильными(если нужно, можно оставить их на английском):
				Ошибка: {Подробное объяснение ошибки}
